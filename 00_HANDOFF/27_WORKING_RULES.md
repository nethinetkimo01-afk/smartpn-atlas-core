...
## 十四、真實生產資料永不進 git（入規27，Jim/中樞定案 2026-07-15，永久標準）

> 起因：`f9126aa`／`fa0906e` 把 IE-VER 對帳報告（`real_db_for_recon/*.md`）當一般文件 commit 進 dev-iever。
> 報告內含**鞋型名稱、ART 料號、標準工時、實際人數、工序明細**＝客戶的真實生產資料。
> 舊 .gitignore 甚至明文寫「DB 檔不進 git（報告 .md 要追蹤）」——**擋住了大檔，卻放行了真資料本身**。
> 根因：防護對象抓錯。防的是「檔案大小/格式」，該防的是「資料是不是真的」。

### 27-1 定義：什麼是真實生產資料

以下任一，皆為真實生產資料：
**鞋型/型號名稱、ART 料號、標準工時（standard_time/normal_time）、
實際人數（actual_operators）、理論人數、工序明細（process_name/part_name）、
EOLR/產能數字、以及任何由真庫（正式庫／ME129／其隔離副本）匯出的統計。**

### 27-2 鐵則

- **真實生產資料永不進 git**，**不論 repo 公開或私有**。
  私有 repo 不是保險箱：協作者、fork、CI log、未來公開化，任一都會外流。
  「現在只有我們看得到」不是理由——**進了 git 就是進了永久歷史**。
- **報告留本機，靠檔案傳遞**（`real_db_for_recon/` 已整個 gitignore）。
  要給中樞看＝貼內容或傳檔，**不要 commit**。
- git 只放**程式碼、schema、規格、閘門**——即「怎麼算」，不放「算出來的真數字」。
- 已誤入者：**改寫該 commit 移除**，不是新 commit 刪除（新 commit 刪除＝歷史裡還在）。

### 27-3 閘門15 `data_leak_gate`（常設，缺跑＝整批不過）

- 斷言對象＝**本次 push 會送出去的新 blob**（remote 還沒有的物件，
  `git rev-list --objects <range> --not --remotes`）。三項斷言：
  - **L1** ART 料號形狀＝**2碼大寫英文＋4碼數字**（真庫實測 140/140 皆此形）
  - **L2** 鞋型清單（執行時從真庫即時取）
  - **L3** `real_db_for_recon/` 底下任何檔案都不得進入任何待 push commit
- **不是只掃 HEAD**：HEAD 乾淨但中間某顆 commit 有真資料 → 那些 blob 仍是 remote 沒有的新物件，
  仍會被抓到。f9126aa 正是此例（已實測會紅）。
- **也不是「每顆 commit 的整棵樹」**：整棵樹會把 remote 早就有的舊 blob 也算成本次罪狀
  → 每次 push 都紅 → 閘門變成永遠擋著的假警報，最後一定被人繞過。
  **假警報是閘門的死因**：擋錯的閘門跟沒有閘門一樣，而且更糟（讓人學會忽略紅燈）。
- **L1 一律嚴格：形狀命中就紅，不因「這顆不在真庫裡」而放行。**
  真庫只有本機這份，ME129／未來新料號不在裡面——**「不在真庫」≠「不是真料號」**。
  代價：規格/程式碼/commit message **不得出現符合 ART 形狀的字串**，
  示例一律寫「2碼英文+4碼數字」。少寫一個假料號範例，換到「新料號也擋得住」，划算。
  實例：`data_leak_gate.py` 與本節初稿都寫了形狀範例 → **被自己的閘門擋下** → 已改文字敘述。
- **鞋型清單不得寫死在閘門檔裡**——寫死＝閘門自己把真鞋型 commit 進 git，
  用來防洩漏的東西自己變成洩漏源。一律**執行時從本機真庫（gitignored）即時讀取**，只存在記憶體。
- CI log 只印**遮蔽後的樣本**（`IE****` / `OZW***`）：閘門的錯誤訊息本身也不該變成外洩管道。
- fail-closed：讀不到真庫 → **exit 2 擋 push**（無法舉證乾淨 ≠ 乾淨）。
- 入規時已實測「先紅後綠」：植入一顆真 ART 料號＋一個真鞋型（**樣本不寫進本檔**，
  寫進來這份規格自己就會被閘門15 擋下——這正是規則對的證據）→ L1/L2 紅；
  未清理的 f9126aa/fa0906e → L3 紅；清理後 → 3/3 綠。

### 27-4 既有污染：入規27 之前就已經洩出去的（2026-07-15 查獲，未結）

> 閘門15 入規當天即查出：**入規27 是在「已經被違反很久」的狀態下才立的**。

- `origin/main`（dbbf2cd，**早已 push 到 GitHub**）底下 **44 個檔案**含真實生產資料樣式，
  包括 `00_HANDOFF/` 多支交接/狀態文件、`data/source_files/IE_MANIFEST.md`（ART 料號 152 處）、
  `docs/preview/SMARTPN_DEMO.html`。**非 f9126aa/fa0906e 造成，是本批之前就有的。**
- 閘門15 對此**只報不擋**：這些 blob 早在 remote 上，擋了會讓每次 push 都紅（見 27-3 假警報條）。
  但**每次跑都會把它印出來**，不准它靜靜過去。
- **處置權在中樞，不在開發者**：清除＝改寫已 push 的 main 歷史 + force-push，
  而 main 目前為**凍結**狀態（裁決：維持 dbbf2cd 不動）。且 git 歷史一旦公開，
  改寫也救不回已被 clone/fork/快取的部分——**須併同評估是否要換 repo / 撤 repo**。
- 記入 `47_GOAL_LEDGER.md` 待中樞裁決，未結前不得自行動 main。
