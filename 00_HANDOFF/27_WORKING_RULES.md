...
### R5 守門/錯誤訊息類測試一律真 subprocess ＋ 真主控台編碼（2026-07-15 入規）

- 凡是驗「擋下時給的訊息/退出碼」的測試，**必須真的 subprocess 跑起來、走真主控台編碼**。
  用 `io.StringIO` 攔 stdout 的行程內假環境**不算數**。
- 據以入規的實例：開機 schema 守門在 `io.StringIO` 下測試全過，
  真主控台（ME129＝cp950）卻在 `print('❌…中文…')` 直接 `UnicodeEncodeError`
  → 守門自己炸成 traceback、exit code 1 而非 3。
  **守門變成現場看不懂的堆疊錯誤，正是守門要防的事。**
- 推論：假環境測「訊息長怎樣」永遠會過，因為 StringIO 吃得下任何 unicode。
  **能不能印出來，只有真的印才知道。**
